...
def eval_storchenbank(client, update: WhatsAppUpdate):
    ...
    if antwort.lower() == echter_wert.lower():
        client.messages.create(
                    body='Du hast die Tafel also entdeckt! Dort werden die Rückkehrzeiten und der Nachwuchs des Storchenpaares festgehalten.'.format(name=update.ProfileName),
                    from_='whatsapp:+14155238886',
                    to=update.From
                )
    else:
        client.messages.create(
                    body='Fast! Neben der Storchenbank findest du eine Tafel, auf der die Rückkehrzeiten und der Nachwuchs des Storchenpaares festgehalten werden.'.format(name=update.ProfileName),
                    from_='whatsapp:+14155238886',
                    to=update.From
                )
